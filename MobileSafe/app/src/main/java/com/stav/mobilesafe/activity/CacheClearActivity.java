        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case UPDATE_CACHE_APP:
                    ...
                    String formatFileSize = Formatter.formatFileSize(getApplicationContext(),cacheInfo.cacheSize);
                    tv_memory_info.setText(formatFileSize);
...
//                                e.printStackTrace();
//                            }
                            //通过查看系统日志，获取开启清理缓存的activity中的action和data
    ...
    private void initData() {
        new Thread(){
            ...
            public void run() {
                ...
                //3.给进度条设置最大值（手机所有应用总数
                pb_bar.setMax(installedPackages.size());
                //4.遍历每一个应用，获取每一个有缓存的应用信息（应用名称，图标，缓存大小，包名）
                for (PackageInfo packageIfo : installedPackages) {
                    //包名作为缓存信息的条件
                    String packageName = packageIfo.packageName;
            ...
            public void onGetStatsCompleted(PackageStats pStats, boolean succeeded) throws RemoteException {
                ...
                if (cacheSize > 0) {
                    ...
                    try {
                        ...
                        cacheInfo.packagename = pStats.packageName;cacheInfo.name = mPM.getApplicationInfo(pStats.packageName,0).loadLabel(mPM).toString();
